---
name: write-feature-artifacts
description: Create feature artifacts: a product requirements doc and a technical implementation handoff doc. Use when the user wants to document a feature we've designed.
---

You have just completed a session where a feature was designed. Now produce two documents in `feature-artifacts/<feature-slug>/`:

1. `product-requirements.md` — business perspective, no technical details
2. `technical-implementation.md` — comprehensive implementation plan for a new session to execute

## Step 1: Determine the feature name

If the user hasn't provided a feature name, ask for one. Convert it to a slug (lowercase, hyphenated) for the folder name.

## Step 2: Create the directory

```bash
mkdir -p feature-artifacts/<feature-slug>
```

## Step 3: Write `product-requirements.md`

Write from the perspective of a product manager. Include:
- **Description**: What the feature does and why it matters to users
- **Acceptance Criteria**: Concrete, testable criteria that define "done"

Do NOT include:
- Technical architecture
- Implementation details
- File paths or code
- API endpoints

Refer to `feature-artifacts/refresh-backlog-covers/product-requirements.md` for style.

## Step 4: Write `technical-implementation.md`

Write as a handoff document for a new coding session. It must be self-contained — the new session should have everything it needs without asking questions. Include:

- **Goal**: One-sentence summary of what we're building
- **Current State**: What exists today, with file paths and line numbers for key code
- **Files to Modify/Create**: Table of all files touched, with action (modify/create)
- **Step-by-Step Instructions**: Numbered steps with full code blocks. Each step should be complete enough that a new session can execute it without guessing.
- **Architecture Decisions**: Table of key decisions made during the grill-me session and their rationale
- **Edge Cases**: Known edge cases and how they're handled

Refer to `feature-artifacts/refresh-backlog-covers/technical-implementation.md` for style.
